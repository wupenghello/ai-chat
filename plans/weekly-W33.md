...
## Code Review 记录（NCR-iter11-004 整改，2026-08-16，待 CEO 过目）

**iter-11 全量 review**（范围 `05b4ac8..dfa093c`，20 文件 +3877/−549；对照 development.md §3 三问）：

1. **范围**：REQ-026/027/028 全量交付，无计划外需求类变更；R1/R2 基线修订均有 CEO 决策记录；「明确不做」四项（REQ-029/移动端/联网搜索/ComposerBox）未被触碰。**无偷换范围**。
2. **隐患**（按风险降序）：① DropdownMenu 互斥用模块级可变状态——单用户 UI 安全，HMR 下有瞬态失响应可能（onBeforeUnmount 已清理监听，刷新即恢复；生产无 HMR）；② 外点吞击 stopImmediatePropagation 会吞同批 capture 监听——当前页面无第三方 capture 依赖；③ SettingsForm 每次打开重拉 /api/quota——频次极低可接受；④ 侧栏收起 localStorage 键无版本命名空间——键名唯一，低风险。**无高危**。
3. **测试**：前端 209→247（新增 timeGroup 6/DropdownMenu 9/弹窗 7 等，组件交互全路径）+ 真实浏览器双轨取证（§7.2 清单 49 条全留档含 auth 端到端）+ 后端 119/119 未动。**覆盖真实**。

**iter-9 追溯 review**（`0a93326..1049e3c`）：REQ-021 改密/注销（ON DELETE CASCADE 级联删除为不可逆数据销毁——已有密码强确认 + 后果文案明示，口径合规）+ 账号 UI + Compose 编排 + 直连死代码删除 + 跨零点 token 归属修复。QA 2 NCR 当时已整改、23 条走查在案。**回顾结论：无已知未修复缺陷遗留**；该迭代 Code Review 缺失属流程缺口（NCR-iter11-004），以本追溯 + 既有证据链补足。
**iter-10 追溯 review**（`fafa487..dbdf98a`）：技术债三项（boot 重试/auth_sessions 惰性清理/useTheme 单测）+ spec hygiene + 首块复测。惰性清理在签发稀疏时过期会话滞留 DB——仅存储占用，无安全影响（token 校验独立于清理）。QA 符合 0 NCR。**回顾结论：低风险清理迭代，无需返工**。

**处置（2026-08-16 闭环）**：CEO 会话过目（回复「继续」确认）；iter-9/10 追溯处置 = 回顾性补足、不返工，CEO 认可。NCR-iter11-004 四条全部关闭。
