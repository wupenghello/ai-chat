...
## ~~待浏览器复核（视觉观感，非功能）~~ —— 已全部复核闭账（2026-08-16 补验）

预览面板端口被并行会话占用后，改用 **puppeteer-core 驱动本机 Chrome 对真实应用全流程走查**（Vite dev + FastAPI 临时库），30/30 断言全过，截图 13 张留档 `/tmp/e2e8/shots/`（关键 6 张已送 CEO）：

1. 后台整页暗色对照（42）：**元素级亮色残留扫描**（.admin-page 全元素 computed bg 亮色检测）0 命中；表格底 = rgb(30,32,38) = surface 令牌逐值吻合 ✅
2. 超长用户名 ellipsis（22）：32 字符中文名 text-overflow=ellipsis、max-width 150px、offsetWidth 150 < scrollWidth 416、title 全名 ✅
3. 封禁确认模态 / 调配额模态 / 解封（14/16/17）：真实点击流——模态文案、确认封禁→胶囊翻已封禁、解封直接生效、调配额双选项 ✅
4. 真实导航（3/4）：未登录 /admin → 登录页；普通用户 /admin → 403 卡（DOM 无盾牌入口）✅
5. 迁移条堆叠观感（37~41）：双条同屏、三要点文案、档案导入完成态、会话导入完成态、本地 IndexedDB 只读备份 3 条保留、30 天清除键已设 ✅
6. **REQ-014 浏览器端全量口径复验**：种入含 2 个 sk-legacy-key 的旧档案 + 3 条旧会话 → 档案导入完成后 localStorage 全文检索不到任何 key（systemPrompt 保留）、云端多出 2 套档案与 3 个会话 ✅ ——「销账/待补」张力解除
7. 被封禁登录横幅（29）：封禁后真实登录 → warning banner「账号已被封禁…」✅

附注（观察项，非缺陷）：① 走查中同 IP 重复注册触发 429——REQ-024 注册限频在真实浏览器生效的顺带实测；② 未登录首访 /admin 落地为 `/login?expired=1`（boot 期 401 失效钩子先于路由守卫落参），登录后回主界面而非回跳 /admin——iter-6 既有行为，design-iter-8 §1.1 走查 3 的字面口径（redirect=）与实测存在细微差异，登记 iter-9 定夺（属登录态探测时序，非 iter-8 引入）。
...
## ~~待浏览器复核（追加）~~ —— 已随上文 2026-08-16 真实浏览器走查全部闭账（5→第 7 条截图 09/12、6→第 8 条 REQ-014 复验）
