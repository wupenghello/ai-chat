...
| # | 结果 | 取证 |
|---|------|------|
| 1 全仓库裸色值清零 | ✅ | grep 审计：组件样式仅剩 `#fff`（实底白字）与深底白色叠层两类合法保留（**初判失准补正**：AppToast 底色 var(--c-text-1) 取巧与 #A3BCFF 裸色当时漏检，由 QA NCR-iter5-001 抓出，按 DEF-013 修复为 --c-toast-bg/--c-toast-border/--c-primary-on-dark，见文末修复走查）；App.vue 命中均为 :root/[data-theme] 令牌定义区 |
...
## 修复走查（DEF-013，NCR-iter5-001 整改后）

- toast 底色 computed：亮 `rgb(35,39,46)`（#23272E）＝修前视觉不变；暗 `rgb(42,45,52)`（#2A2D34，--c-toast-bg 暗色值）
- toast 描边：亮与底同值不可见（外观不变）；暗 `#33363E` 分层
- 动作链接色 `var(--c-primary-on-dark)`＝#A3BCFF，两模式 toast 均深底、对比度不变
