...
零改写（+14 例全为新增；既有 364 例零改动复跑全绿，功能性删除为零）。

### 走查（验收 6，scripts/e2e-walkthrough-18.mjs，2026-08-21）

**结论：31 条清单 41 断言 PASS / 1 FAIL（1 个真产品缺陷 DEF-038）+ 8 条交叉引用承载。** 脚本落盘 scripts/e2e-walkthrough-18.mjs（三后端两前端：主后端 8818 真 Tavily / 禁用态后端 8820 空 search_key / time_limit 小值注入复用主实例改 env；两 vite 5182/5183），截图 9 帧 /tmp/e2e18/shots。

**走查覆盖**（浏览器断言 22 条 + 交叉引用 8 条 + 1 缺陷）：
- A 开关：条 1~10 PASS（M38/M39 逐字 + 36×20 几何 / 信息行布局 / 关闭态默认不持久化 / 开启态 M40 / 禁用态真实空 key 后端 M41/M42 不隐藏 / 可用性翻转 / 发送复位 + mode=research 载荷 / 关闭态无 mode / mode 不写会话档 / 生成中交互）；**条 11 FAIL = DEF-038**。
- B 进度（research 端到端真 Tavily）：条 12~17 PASS——发「Rust 在后端开发中的采用情况如何？」→ mode='research' + 开关发送即复位 + 计划文本流式 + 5 张搜索工具卡（运行中→完成 + 创建即折叠）+ 5 引用卡 + 报告（[n] 无链接无角标）+ blocks 顺序零重排 + 生成 hint；条 18~20 交叉引用（vitest 承载）。
- C 到顶：条 22 PASS（time_limit 小值注入真实触发 → M43「已到研究时长上限」pill + 已生成 1660 字保留）；条 21/23~26 交叉引用（vitest/存量走查承载）。
- D 数据面：条 27~31 PASS（quota 字段双向 / 刷新时机 / 暗色全 var() / 样件虚构 / 无 phase 条无模式徽标无切换 toast 反向断言）。

**真 Tavily 随测三连收口（T0 §1 登记项）**：① [n] 引用对应真实来源 PASS——报告 [1]~[12] 标注 25 个真实 Tavily 来源（5 搜索 × 5 条），无编造声明；② 报告正文 ≤3000 字 PASS——全文 2705 字；③ 检索结果质量 PASS——结论先行 + 分点论证、事实有依据、无「未搜到/编造」声明。

**缺陷 DEF-038（条 11，CEO 2026-08-21 定夺「改规格对齐 DOM」）**：Tab 序与规格不符——实现 DOM 序 = textarea → 发送钮 → 开关（开关挂在底部信息行 `.composer-hint`，DOM 序天然在发送钮后），design-iter-18 §7.2 条 11 原声明「开关先于发送钮」。焦点环本身正确。处置 = 改规格：条 11 表述改为「textarea → 发送/停止钮 → 开关」（design-iter-18 R1 + index.html §7.2 + 走查脚本断言同步），实现零改动。走查结论修正为 **41 PASS / 0 FAIL**（条 11 由「规格矛盾 FAIL」转为「规格修订后 PASS」）。

**随测观察（非缺陷）**：报告 [n] 用跨搜索累计编号 [1]~[12]（25 真实来源），而 R2 指令「n = 该次搜索结果来源序号」语义倾向单次搜索内 [1..5]——所有来源真实无编造（REQ-035 精神满足），但跨卡累计编号与单卡 5 条的对应关系对用户有轻微歧义。候选观察项（后续若需可在暂缓池立「research 引用编号口径」）。
