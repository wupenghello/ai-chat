...
**缺陷 DEF-038（条 11，当轮不改代码）**：Tab 序与规格不符——实现 DOM 序 = textarea → 发送钮 → 开关（开关挂在底部信息行 `.composer-hint`，DOM 序天然在发送钮后），design-iter-18 §7.2 条 11 声明「Tab 序 = textarea → 开关 → 发送/停止钮」。焦点环本身正确（`rgba(51,112,255,.12)` 3px 已实证）。根因 = 规格 Tab 序表述与开关「底部信息行」落形（§2.1 定案）矛盾，待 CEO 定夺：改实现（tabindex 重排，焦点先到下方开关再到上方发送钮视觉别扭）vs 改规格（条 11 表述对齐实际 DOM）。
