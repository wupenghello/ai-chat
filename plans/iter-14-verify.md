...
## 验收走查反馈整改（CHG-008 + DEF-034，2026-08-18 CEO 真机验收后当轮处理）

### 1. 反馈与处置对照

| CEO 反馈原文 | 处置 | 落点 |
|---|---|---|
| 「查询到过程中，search 不需要展开」 | 折叠规则 R1 修订：工具卡/引用卡**创建即折叠**（含运行中，徽章示进度），展开由用户；design-iter-14 §12 + components.md v1.4 登记 | `src/components/ToolStepCard.vue`；走查条 39① 断言随改（运行中帧断折叠态） |
| 「渲染过程中拉动页面滚动不行，页面持续往下走」 | **DEF-034**：程序滚底的 scroll 回声重置跟随标记 + 120px 阈值内用户抬升即被增量拽回（双机制叠加致脱离不可达）→ 回声忽略 + 距底 >120px 即脱离 + 回底自动恢复 + 「↓ 回到底部」浮钮（零高度 sticky 槽，零新令牌） | `src/components/MessageList.vue` |
| 「时间不对，搜索结果一周以上延迟；问系统时间也不准」 | ① **REQ-033 增补**：系统段恒含当前时间行（模型无时钟）② **REQ-035 增补**：search 入参可选 `days`（1~30，模型自行时效限定，透传 Tavily topic=news+days）；网关参数校验器补数值边界（minimum/maximum） | `backend/app/agent.py`（_now_line/assemble_context）、`backend/app/search.py`、`backend/app/tools.py` |

### 2. 测试证据（CHG-008 后全量复跑）

- **后端 pytest：220 → 224 全绿**（新增 4：时间行 2（格式正则/无提示词仅时间行）+ days 3——透传 news+days、缺省不带、越界 31 网关拒绝零连接；实为 5 新例中 1 例合入既有分组）+ ruff clean；既有「组装等价」用例按新系统段口径改写（system 段含时间行，逐字段等价改为首段正则 + 其余逐字），映射登记于用例注释。
- **前端 vitest：301 → 305 全绿**（R1' 适配 3：运行中折叠可展开/用户态保持/参数块需展开后读；滚动跟随新增 4：贴底跟随/脱离不拽底+浮钮回底/**回声不重置根因**/手动回底自动恢复）+ guard:style + 生产构建通过。
- 走查脚本条 39① 断言更新（R1' 折叠态），**全量走查已重跑复验：56 PASS / 0 FAIL（2026-08-18 复跑，真实 Chrome + 真实 Tavily 端到端，R1' 断言下全过；日志 /tmp/e2e14-rerun.log，截图更新 /tmp/e2e14/shots/）**。

### 3. 变更登记

changes.md **CHG-008**（CEO 指示原文三项 + DEF-034 随附；影响评估含工作量 S×3 与用例映射）；defects.md **DEF-034**（根因：回声重置 + 阈值覆盖双机制）；design-iter-14 **§12 基线后修订记录**（R1' + days）；spec REQ-033/035 条款增补随 CHG-008；RTM REQ-033 整行收口 + 全局回归基线 305+224。
