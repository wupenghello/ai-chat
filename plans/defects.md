...
| DEF-011 | 2026-08-15 | ComposerBox 输入区实现偏离已基线设计稿 iter-1（4.6 节）：布局由「textarea 在上 + 底部操作栏（hint + 发送按钮 36px）」改为「textarea 与按钮并排单行 + align-items:flex-end」；textarea 字号 15px（基线 14px/1.6）；容器 padding 10px 10px 10px 16px（基线 12px 12px 8px）；缺 shadow-1 阴影；缺底部「Enter 发送 · Shift+Enter 换行」hint；textarea 无自身内边距。现象：单行输入文字偏上、与发送按钮不对齐，CEO 反馈「输入框输入内容位置歪歪扭扭」 | 一般 | 待修复 | 处置（回基线设计稿 iter-1 4.6 节）：布局改回两行结构（textarea 上 + 底部 bar：hint + 发送按钮 36px）；textarea 字号回 14px/1.6；容器 padding 回 12px 12px 8px；补 shadow-1 阴影；textarea 补内边距。关联 CHG-002 |
