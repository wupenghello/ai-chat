...
- 内容（对 spec 的拟改，spec 级详细度——沿 CHG-007/009 呈批详度口径（CEO 2026-08-17 定），批准后按拟文落 spec/RTM。除内容 4 汇总的定夺项外，其余口径本 CHG 定死）：
  ...
  4. **定夺项清单（2026-08-19 全部定夺，CEO 原话「全部按推荐批准」；推荐即定案，理由摘要留档）**：
     ...
     | ③ | 自动压缩 token 阈值 | **「T0 取证后定死」处理方式**（沿 B1 缓存字段冒烟取证先例）：T0 实测 30 轮会话 prompt_tokens 基线 Y → 阈值定死 0.75Y；**实施期暂以 32768 占位**（现状无 tokenizer，拍脑袋阈值会失真且违铁律 5 度量精神） |
     ...
     | ⑦ | 30 轮验收的 X 值与测量法 | **X 随 T0 取证定死（与③同源：X = 0.75Y 口径下的请求体上限，T0 报告定精确值）；测量法 = pytest MockTransport 捕获第 31 次请求体 + 假上游 usage 帧 prompt_tokens 机器读数（不手数 token，铁律 5）+ 关键信息问答断言；另真实 DeepSeek 30 轮冒烟走查留档 T0**（审核稿 B2 验收口径原话「X 与测量法随稿定死」的落实） |
...
- 落地核对清单（v1.4.10 制度 A；CEO 批准后逐项落地勾验，首个任务提交内落盘）：
  ...
  | 6 | 定夺项①~⑨结论回填本条（含③⑦的 T0 取证定死值随 B2 T0 报告回填） | ✅ 已落地（2026-08-19；③⑦精确值待 B2 T0 取证回填） | 状态行 / 内容 4 定夺表 / CEO 批准字段 |
  | 7 | registry.md 同步（主会话执行） | 待主会话执行 | — |
