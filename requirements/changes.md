...
## CHG-018 deep-research 深度升级：方法论重写 + read 工具 + 深度档位（CEO 线上验收反馈直派插队批次）

- 日期：2026-08-23
- 类型：修改（REQ-046 正式改写——研究方法论与完成判据、报告结构、引用坐标、护栏档位化）+ 新增（REQ-054 read 工具 / REQ-055 深度档位；REQ-047/030 波及注记）
- 状态：**已批准（2026-08-23 CEO 直派三定夺：① 完整升级（prompt 重写 + read 工具 + 护栏再校准）② 成本口径 = 做深度档位（轻量/标准/深度三档，用户发起时可选）③ 排期 = 插队直派（沿 CHG-008/CHG-017「CEO 指示即批准」先例，与在途批次并行登记不打断提交序列）；实现级定夺由 T0 取证定死——见下「定夺项」与 plans/chg-018-verify.md）**
- 原因/依据：CEO 线上使用 deep-research 后验收反馈（2026-08-23）：「从逻辑上来看没什么问题，但是我看也没什么深度啊，感觉仅仅只是在解决需求的基础上做到的最小程度的深度研究」。T0 取证坐实（verify §2）：iter-18 R2 定稿 RESEARCH_PROMPT 的完成判据（「每个子问题**至少**检索一次 + **全部子问题覆盖后**输出报告」）把及格线写成天花板——单遍覆盖、无交叉验证、无矛盾呈现、无遗留问题段；工具面只有 search（5 条 snippet）模型从未读过原文；护栏 16 步按常态 3~5 步校准从未成为约束。**规格使然，非实现走样**。随批闭账 iter-18 T0 §1 三项随测（verify §8）。
- 内容：
  - **A. 研究方法论重写（REQ-046 改写）**：RESEARCH_PROMPT 单文案 → **三档文案**（逐字断言面 = verify §9 R2 定稿）。方法论从「单遍覆盖」升级为「轮次推进」：标准档两轮（广度轮多角度拟词检索 → 深度轮对薄弱点/单一来源/矛盾处换词重检交叉验证）；深度档三轮（广度 → **深读轮 read 读原文、数字/日期/版本号从原文核对** → 验证轮）；轻量档快速核实（2~4 子问题 + 单一来源如实标注）。**完成判据从「子问题覆盖」改为「证据质量」**（关键结论 ≥2 相互独立来源支撑、矛盾已查证或如实标注分歧）。**报告结构四段**：结论（含置信度高/中/低与理由）/ 分点论证 / 矛盾与分歧 / 未能验证的遗留问题（轻量档精简口径）。**引用坐标 [m-n] 全档统一**（m = 第 m 次工具调用、n = 该次结果来源第 n 条；read 单来源标 [m-1]）——消除旧 [n] 跨轮歧义（随测①闭账结论，verify §3 R1 发现 2）。**报告字数上限按档**：light ≤1600 / standard ≤3500 / deep ≤7000（随「矛盾+遗留」结构成本调整，实测达标 verify §4）。
  - **B. read 工具（REQ-054 新增，第三个生产出网工具）**：Tavily `/extract` 端点（同域 api.tavily.com——**出网白名单零扩大**，SSRF 白名单 + DNS 解析期核验照旧）；单 url 必填参数（maxLength 2000）；**内部截断 10000 字符**（防上下文溢出：read 全量 32KiB × 6 次可逼近上下文上限，verify §5）+ 网关 32KiB 外限沿用；单工具超时 15s（extract 慢于 search，T0 实测留余量）；gate 沿 "search"（三与门共用——admin 开关 ∧ key ∧ 档案工具开关，不新增开关）；**仅 research 回合下发**（普通回合 tools 定义零变化——沿「普通回合逐字节等价」铁律）；结果双视角：文本给模型（标题+URL+原文）+ sources 单来源数组给前端引用卡（tool.result 可选字段既有机制）；failed_results/空正文 → error result 降级（模型可换来源重试或直答，回合不崩）。
  - **C. 深度档位（REQ-055 新增）**：turn 端点请求体加法可选字段 `depth`（Literal["light","standard","deep"]，缺省 "standard"；mode='chat' 时忽略——宽容口径）。**护栏按档参数化**（verify §6 T0 定死）：light 8 步/300s、standard 16 步/900s（**现值零变化**）、deep 32 步/900s；config 新增 4 参数（research_steps_light/research_timeout_light/research_steps_deep/research_timeout_deep），既有 max_research_steps/research_total_timeout 语义收窄为标准档（默认部署零变化）。**research 回合上游载荷带 max_tokens=8192**（T0 发现生产隐患：现网不带 max_tokens、DeepSeek 默认输出 4096 tokens，7000 字报告必然截断——verify §3 发现 4；普通回合载荷零变化）。前端 ComposerBox 深研开关扩展档位选择（开启态出档位小选段，文案/hint 随档；发送即复位沿定夺③体例），进度条 max_steps 动态呈现既有机制零改动；mode/depth 均回合瞬时属性不写会话档。
  - **D. 计费与成本口径（CEO 定夺②量化落档，verify §7）**：1 发起 = 1 回合口径零变化（REQ-034）；tokens 如实累计——实测 light ~24k / standard ~89k~104k / deep ~93k~146k tokens/回合（旧 prompt 真数据 ~67k）；Tavily credits light ~4 / standard ~9~10 / deep ~9~14 每回合（免费 1000 credits/月 ≈ 标准档 100 次）。
- 定夺项（CEO 计划会话三定夺 + T0 实测定死的实现级定夺）：
  - **① 改进范围 = 完整升级**（prompt 重写 + read 工具 + 护栏再校准；CEO 2026-08-23）。备选「仅 prompt」「先取证再定」不取——工具面无读原文能力则深度有 snippet 上限（T0 深度档 read 行为验证：三轮形态完整呈现，verify §3）。
  - **② 成本口径 = 深度档位**（三档用户发起时可选；CEO 2026-08-23）。暂缓池「deep-research 独立开关与用量可见性」**维持留池不收编**（CEO 定夺：做档位不做可见性）。
  - **③ 排期 = 插队直派**（CEO 2026-08-23，沿 CHG-008/CHG-017 先例）。
  - **④ read 下发面 = 仅 research 回合**（实现级，T0 后定）：保「普通回合工具面逐字节零变化」铁律；备选「全模式可用」不取（改变普通回合 tools 载荷，违背零回退原则，且 read 的价值场景即深研）。
  - **⑤ 引用坐标 = [m-n] 全档统一**（实现级，T0 R1 实证模型自发优选该形态且消除跨轮歧义；旧 [n] 口径随 REQ-046 改写退役——前端零改动，[m-n] 为纯文本渲染）。
  - **⑥ max_tokens=8192 仅 research 回合**（实现级，T0 发现的生产截断隐患修复；普通回合零变化）。
  - **⑦ 前端形态 = 开关 + 开启态档位小选段**（零设计基线前置：既有 ToggleSwitch + segmented 控件家族复用、无新交互模式，沿 CHG-017「视觉参数级改版 + 既有模式复用零设计稿」先例；文案 M 常量逐字随实现定稿登记 verify）。
- 影响评估：
  - 受影响需求：**REQ-046（正式改写：描述/主流程/验收 1/异常「报告超长」分支随档位口径改写，spec 同批）**；REQ-047（波及注记：入口交互扩展档位选择、hint 文案、进度 max_steps 动态——呈现面零新增）；REQ-030（波及注记：research 回合 max_tokens 载荷加法，普通回合零变化）；REQ-035（波及注记：search 工具本体零改动，read 与其同域同门控不同端点）；REQ-054/055（新增）。
  - 测试影响：tests/test_research.py 逐字断言面随三档文案改写（受控映射登记）；test_search.py read 工具用例新增（网关六项校验 + SSRF + 截断 + gate 矩阵，沿 REQ-035 体例 +10~14）；档位矩阵用例（depth 三档 × 护栏参数 × mode 交互 + max_tokens 断言 +6~8）；前端 vitest 档位选择/hint/复位 +4~6。预计 pytest 363→~385、vitest 435→~440。
  - 交互原型与设计稿同步：零设计基线前置（定夺⑦）。
  - 风险：深度档 token 消耗 ~146k/回合（上下文累积——DeepSeek 128k 窗口内实测安全，read 内部截断 10000 字符为防线）；报告前 1~3 句过渡性元叙事为已登记接受行为（流式进度反馈价值，verify §3 R1 发现 3）；旧会话历史中的旧 [n] 引用标注不受影响（纯文本，读时无解析依赖）。
- CEO 批准：直派（2026-08-23 计划会话三定夺 + 计划批准；T0 取证与实现级定夺④~⑦留档 verify）。

## CHG-017 主题「自动」档 + 设置弹窗账号分区改版（CEO 上线后样式走查直派批次）

- 日期：2026-08-23
- 类型：修改（REQ-017 增强 + 设置弹窗账号分区 UI 改版；同批伴随四项样式缺陷 DEF-043~046 当轮修复，缺陷面见 plans/defects.md）
- 状态：**已批准（2026-08-23 CEO 直接指派并验收口径 =「样式优化」六项清单；本条登记其需求级两项，缺陷级四项走 DEF 登记）**
- 原因/依据：CEO 上线后实际使用反馈六项样式问题（2026-08-23 直派）：①设置弹窗分区标题 border-top 去除（DEF-043）②弹窗显示内容尺寸过大不匹配、输入框/按钮突兀（DEF-044）③文件卡与段落间距混乱（DEF-045）④账号分区设计/布局/交互重构（本条 A）⑤外观分区增加「自动」选项（本条 B）⑥整页可按住拖动晃动（DEF-046）。
- 内容：
  - **A. 账号分区改版**（SettingsForm 分区七）：收起式三段布局——账号身份卡（首字符头像 + 用户名 + 成员/管理员标识，此前弹窗内无任何当前账号身份呈现）→「修改密码」分节（收起态仅一行说明 + 「修改」入口钮，展开才渲染三字段并自动聚焦旧密码；取消/Esc 收起即清空，成功即收起留成功横幅；改密三字段校验/后端口径/dirty 拦截链零变化）→「注销账号」危险分节（静默样式：danger 描边幽灵钮替代原 danger-l 大红块；DeleteAccountModal 二次确认强模态不变）。Esc 链插入点：未保存确认 > 档案编辑模态 > 记忆编辑态 > **改密表单展开态** > 注销/删除确认 > 外层弹窗。
  - **B. 主题三档化**（REQ-017 增强）：`ThemePref = 'light' | 'dark' | 'auto'`——auto = 跟随系统 `prefers-color-scheme`（模块级 mql 监听，系统切换实时生效、仅 auto 档消费；显式浅/深档不被系统事件覆盖）；`resolvedTheme` 计算值承载实际生效主题（登录页翻转钮图标/提示改按生效值判断）；持久化存档位值（auto 不落解析值）；matchMedia 不可用兜底浅色（沿 useMediaQuery jsdom 口径先例）。外观分区 segmented 三钮（浅色/深色/自动）+ 一行说明。存量用户存储值 light/dark 语义不变；无存储值默认仍浅色（既有验收口径零变化）。
- 影响评估：
  - 受影响需求：REQ-017（描述/主流程/验收随本条注记扩展 auto 档，spec/rtm 同批）；REQ-021（账号管理 UI 形态改版，功能口径——改密校验、注销强模态、生成中注销终止——零变化）；REQ-028（设置弹窗容器与分区导航零变化，账号分区内部改版）。
  - 测试影响：vitest 429 → **435**（useTheme 新增 5 例：auto 初始化/系统深色解析/auto 实时跟随/auto 档 toggle 翻转/持久化往返；settings-form 新增 1 例：改密展开态 Esc 先收起不关弹窗；既有改密 8 例随按需展开交互改造挂载方式、成功例断言随收起行为改写——功能断言零削弱）；CSS 契约新增 1 例（分区标题无 border-top）。走查：scripts/e2e-style-0823.mjs 真实 Chrome **19/19 PASS**（含 auto 档系统切换实时生效、账号分区三段布局、注销链路端到端自清理）。
  - 交互原型与设计稿同步：零设计基线前置（CEO 直派样式走查批次，触点均为已基线分区的视觉参数级改版 + 既有模式复用〔分节行式 = tools-field/as-row 家族、身份卡 = primary-l 令牌〕；沿 iter-19「零 UI 新模式无需设计稿」先例口径）。
  - 风险：auto 档依赖 mql change 事件（主流浏览器全支持；jsdom 未实现已兜底）；改密按需展开属交互改版，dirty 判定仍以「字段非空」为准（收起即清空 = 放弃输入，语义显式）。
- CEO 批准：直接指派（2026-08-23，六项样式优化清单第 ④⑤ 项）。

## CHG-016 真实天气工具（和风天气 API 接入，第二个生产工具；含 demo_weather 处置）

- 日期：2026-08-22
- 类型：新增（含 REQ-030 波及注记——demo_weather 去留为定夺①；无存量 REQ 行为语义改写）
- 状态：**已批准（2026-08-22 CEO「全部按推荐」——整体批准 + 六定夺全按推荐定案：① demo_weather 移除（echo 保留）② key 与 Host 均配置即启用、不新增 admin 开关 ③ 实时 + 三日预报单工具一次返回（location 单参数）④ 零 UI 无需设计基线（沿 iter-19 先例）⑤ iter-22 独立小迭代 Σ3~4 不顶格、T0 冒烟三项已随凭据复验前置完成 ⑥ CEO 在和风控制台为凭据启用 GeoAPI 服务、不可启用则回退内置约百城 LocationID 表 + 模型坐标兜底；spec/RTM 同日同批落盘，tag req-baseline-v13 由主会话执行）**
- 原因/依据：v1.0.0 已上线（2026-08-22），CEO 提出 demo_weather 为上线后待解决问题第一项。spec §4 暂缓池「真实天气工具」条目拉项（L1112）：CEO 2026-08-17 确认为 A2 候选（候选非承诺），2026-08-18 iter-14 计划定案「不纳入，B1 排期时再评估」——**该触发时点已过**（B1/iter-15 已于 2026-08-19 关闭，其后排期被 B2/C/D1/D2/移动端/用量面板顺次占用），现由 CEO 重新拉起走 CHG。CHG 编号顺延 = **CHG-016**，批准后出基线 req-baseline-v13，新增 **REQ-053**（REQ-052 为当前最大号；REQ-044 永久留档不复用）。
- **现状代码取证（2026-08-22 逐项核实，非推测）**：
  - **demo_weather 现状**（backend/app/tools.py L37-43/L247-275）：固定五城假数据 + 200ms 模拟延迟，无出网，`admin_only=True`（design-iter-13 定夺④）——普通用户不可见，仅 admin 自检；CEO（唯一 admin）自问天气即得假数据，即本问题本体。断言面：REQ-030 验收 6 + tests/test_agent_tools.py 4 处（L19 枚举/L39 执行/L119·L125 注册表与 tools payload）。
  - **生产工具范本全在线**（iter-14 T2，backend/app/search.py）：bind/unbind 运行时绑定（main.create_app lifespan）+ 白名单零连接拒绝（ensure_egress_allowed）+ DNS 解析期地址核验（_assert_public_resolution）+ ToolError 机器可读降级（error result → 模型降级直答，回合不崩）+ gate 注册过滤（tools.py tools_for_user L88-104）。真实天气工具可整体沿此模式，网关六项校验链零改动。
  - **和风凭据已验证备存**（plans/iter-14.md L49 补记，2026-08-18）：凭据 ID CMGTQUUCJA（API KEY 鉴权）+ 专属 API Host；冒烟实证 = 专属 Host `/v7/weather/now` code 200 真实数据；通用 `devapi.qweather.com` 返回 403 Invalid Host（**该账户为专属 Host 制度——出网白名单必须取自配置注入，无公共域可写死**，与 search 固定常量 TAVILY_HOST 的差异点）；凭据已存 backend 侧 .env 不入 git，变量名 `AI_CHAT_WEATHER_KEY`/`AI_CHAT_WEATHER_HOST` 已预留（plans/iter-15-verify.md L16），「B1 启用零等待」。生产服务器 .env（47.79.228.253）未配置（Tavily 2026-08-22 已配，和风未配）——部署动作随迭代交付。**2026-08-22 凭据复验（CEO 同日补发 Host/Key，T0 冒烟前置执行；本地 .env 存量已在 iter-18 key 击穿回退中丢失，本次重存两变量）**：① `/v7/weather/now` 200 真实数据（北京 雾 27°C/体感 31°C，obsTime 当分钟）② `/v7/weather/3d` 200 三日预报完整 ③ **GeoAPI `/v2/geo/city/lookup` 在专属 Host 与 geoapi.qweather.com 均 404 空体（中文/ASCII/top 变体全 404）——该凭据未启用 GeoAPI 服务，「城市名→LocationID」两步链前提当前不成立（定夺⑥承载）** ④ 坐标直查 `location=116.41,39.90` → 200（东城站真实数据）——坐标路径可用 ⑤ 响应无 X-RateLimit 头，额度可见性仅和风控制台。
  - **零新增承载面**：工具执行已落 kind='tool' 遥测行（含工具名）→ admin 工具用量视图自动涵盖（REQ-038 聚合零改动）；前端 ToolStepCard 通用渲染任意工具（四态徽章 + R2 折叠结果卡）→ 天气工具卡零前端改动；配额沿 REQ-034 回合计（工具调用为回合内步，不新设计量）。
  - **admin 开关面成本**（若定夺②选三与门模式）：PUT /api/admin/settings 为 StrictBool 必填单字段形状（routers/admin.py L44-50/L258-266）+ AdminView 开关行 UI + overview 读字段——加 weather 开关 = 端点形状改写 + 非零前端面 + vitest。
- 内容：
  - **REQ-053 真实天气工具〔CHG-016〕**（用户故事：作为用户，我要问天气时 AI 调用真实天气数据源给出实时天气与短期预报，以便得到可信、结构化、不过时的天气回答。描述：第二个生产出网工具 `weather`（和风天气 QWeather，凭据见现状取证）经工具网关接入：**出网白名单 = 配置注入的专属 Host**（.env `AI_CHAT_WEATHER_HOST`；白名单域随配置动态——账户为专属 Host 制度，ensure_egress_allowed 传参化白名单零签名改动，SSRF 判定逻辑零变化），key 服务端 .env 注入（`AI_CHAT_WEATHER_KEY`，与 AI_CHAT_SEARCH_KEY 同法；不入 git/日志/响应体）；**城市定位 = 工具内两步链**（GeoAPI 城市查找 → LocationID → 天气端点；两次子调用均过白名单 + DNS 解析核验；单工具总超时 10s 由网关 wait_for 兜底；**GeoAPI 服务启用随定夺⑥——2026-08-22 复验该凭据未启用 GeoAPI，不可启用则回退内置常用城市表 + 模型坐标兜底〔坐标直查已验证 200〕**）；**数据范围 = 实时 + 三日预报一次返回**（定夺③推荐，location 单参数）；**可见性 = 全员**（非 admin_only），**门控 = key 与 Host 均配置即启用**（定夺②推荐：外层沿「档案『支持工具』开关 ∧ 统一 key」既有工具面门；不新增 admin 开关）；结果为纯文本组装（城市/天气现象/温度/体感/风/湿度/三日概览 + 数据更新时间；逐字文案实现期定稿登记 verify，沿 EMPTY_RESULT_TEXT 先例；无 sources 数组——天气为数据非引用）；步数/断连/截断/注入防护全沿 REQ-030/031 既有护栏零新口径；**demo_weather 处置随定夺①**（推荐移除，echo 保留为无出网自检工具；REQ-030 验收 6 与 test_agent_tools 4 处断言面改写映射逐条登记，沿 test_proxy 16 例退役映射先例）。主流程 = 模型判定天气问题 → tool.call(weather, {location}) → 城市查找 → 天气端点 → 文本组装回填 → 模型综合回答（前端工具卡复用零改动）。异常分支 = 城市查无 → error result「未找到该城市」→ 模型降级（直答或转 search）；API 失败/超时 → error/timeout 降级直答、回合不崩；key/Host 未配置 → 工具不注册（模型不知其存在，用户无感）；结果超限 → 网关截断；配额/步数护栏沿 REQ-030/034 通用分支。验收标准：① 真凭据端到端——真实 Chrome 走查「查北京天气」返回真实数据留档（沿 iter-13 北极星链路形态）② 城市查无/API 失败（假端点）→ 降级直答不崩（pytest）③ key 缺失 → 注册表无 weather、上游请求 tools 定义不含 weather（pytest 断言 payload，沿 REQ-035 验收 3 体例）④ 出网仅专属 Host：目标改内网/他域 → 零连接（pytest，沿 REQ-035 验收 4 体例）⑤ 请求 Host 与配置 Host 不一致 → 拒绝（白名单动态化新模式的 SSRF 专项断言）⑥ 定夺①落地映射——demo_weather 移除后 REQ-030 验收 6 改写 + test_agent_tools 改写映射逐条登记（非静默删除）。优先级 P1；涉及页面 = 不涉及（纯后端；admin 面零改动随定夺②推荐口径））
- 定夺项（呈报 CEO，均附推荐）：
  - **① demo_weather 去留**：推荐 = **移除（echo 保留）**——真实工具在场后 admin 侧两天气工具并存，模型选择面混淆（两工具描述均为「查询城市天气」，可能命中假数据工具——CEO 自问天气得假数据正是本问题本体）；demo_weather 历史使命（A1 无出网验证 ReAct 循环与工具卡渲染）已由真实工具完全覆盖，echo 足以承载无出网自检。备选 = 保留仅 admin（暂缓池条目原口径）——真实工具在场后自检价值趋零，不推荐。
  - **② 门控形态**：推荐 = **key 与 Host 均配置即启用，不新增 admin 开关**——search 三与门的运营止血价值（额度尽即时关闭）对天气不成立：和风额度尽时 API 自身返错 → 工具 error → 模型降级直答，天然自愈，无紧急止血需求；省端点形状改写 + AdminView 开关行 + vitest（现状取证第 5 条）；部署者移除 key 重启即完全下线（部署级控制，单部署者场景充分）。备选 = 沿 search 三与门模式（admin 开关 ∧ key 配置，一致性最高，但引入非零前端面，+S~M）。
  - **③ 数据范围**：推荐 = **实时 + 三日预报单工具一次返回**（location 单必填参数 maxLength 50；「今天天气」与「未来几天」两类自然问题一次覆盖；不设 days 类可选参数——减少模型参数误用面）。备选 = 仅实时（更小，但「这周天气」类问题覆盖不了）。空气质量（/v7/air/now）与气象预警（/v7/warning/now）不入本期，需求浮现入池。
  - **④ 设计基线前置**：推荐 = **无需设计基线**——零 UI 改动（工具卡四态徽章/R2 折叠全复用，admin 面零改动〔定夺②推荐口径〕），沿 iter-19「零 UI 无设计稿任务」先例，不构成 tailoring 偏离。备选 = 增加天气专属卡片形态（文本结果折叠卡内可读性已够，视觉增益有限；若做则 +M2 且 design 基线前置）。
  - **⑤ 排期承载**：推荐 = **iter-22 独立小迭代（Σ3~4 不顶格）**——T0 取证复核 S1（专属 Host 现有效性 + GeoAPI 城市查找 + /v7/weather/3d 两端点补冒烟——2026-08-18 仅验过 /weather/now + 逐字文案定稿 + 免费额度口径核实；**其中冒烟三项已于 2026-08-22 随凭据复验前置完成（见现状取证第 3 条），T0 余量 = 逐字文案定稿 + 定夺⑥回退路径验证**）；T2 后端与走查 M2（weather.py 模块 + 注册 + config +2 变量 + gates 加 weather 键 + pytest +10~15 + e2e 走查 3~5 条）；T1/T3 无（零前端零设计稿）。排期细节走 /mm-iteration-plan 由 PM 起草。备选 = 与暂缓池其他小项搭班（主题不相关，违背「一个迭代一条主线」先例，不推荐）。
  - **⑥ 城市定位路径（GeoAPI 404 取证后新增，2026-08-22）**：推荐 = **CEO 在和风控制台为该凭据启用 GeoAPI 服务（免费订阅通常已含，控制台项目设置操作）→ 维持工具内两步链**——任意地名（含县级/海外）可查，长期最稳，实现面与 CHG 原设计一致；启用后 T0 复验一次即闭环。**若控制台不可启用 → 回退定案 = 内置常用城市表（约百城 LocationID 静态映射，随代码走版本管理）+ 模型直传坐标兜底**（坐标端点已验证 200；大中城市模型坐标可靠，小地名靠城市表覆盖）。备选 = 仅模型直传坐标（零 GeoAPI 零城市表，实现最小——小地名坐标易偏导致查错城市，不推荐）。
- 影响评估：
  - **工作量**：Σ3~4（T0 取证 S1 + T2 后端与走查 M2；零前端零迁移零设计稿）——沿 iter-13~21 定级口径（S=1/M=2/L=3~4），不顶格。
  - **进度**：iter-21 已关闭、下一候选本就待 CEO 定夺（registry 2026-08-22），本 CHG 即候选答案；不影响已发布面（纯加法 + demo_weather 移除为定夺①批准项）。
  - **受影响的其他需求**：REQ-030 波及注记（验收 6「echo 与 demo_weather 各 1 用例」随定夺①改写为 echo + 移除映射登记；描述段「交付 2 个内置演示工具」注记）；REQ-031 复用注记（六项校验链零改动，第③项白名单实例化为「配置注入域」新模式）；REQ-035 复用注记（生产工具模式范本：bind/unbind/降级/逐字文案先例）；REQ-025 零波及（定夺②推荐无 admin 面）；REQ-033/034/038 零波及（组装/配额/遥测聚合零变化，工具行自动入账）。零 REQ 删除、零 REQ 语义改写（REQ-030 为验收条款注记级改写）。
  - **测试影响**：pytest 预估 +10~15（网关面 4~5 + 工具面 4~5 + 门控注册面 2~3；360 存量零静默改动——test_agent_tools 4 处断言面随定夺①改写映射逐条登记）；vitest 零新增零回退（零前端）；走查新增 e2e-walkthrough-22 天气链路 3~5 条（真凭据）。
  - **交互原型与设计稿同步**：零同步（零 UI 波及；design/proto 与 design/iter-* 均无涉）。
  - **风险**：① 和风免费订阅额度（凭据 CMGTQUUCJA 免费档日限——T0 核实额度口径；超限自愈降级见定夺②）② 白名单动态化新模式（配置 Host 注入白名单元组——SSRF 判定逻辑零变化，验收 5 专项断言；Host 配置缺失/非 https → 工具不注册 + 启动 warning，防御性兜底沿 search「理论不可达」体例）③ GeoAPI 两步链失败组合（查无/查到但天气端点失败——统一 ToolError 降级，pytest 覆盖；若 GeoAPI 在专属 Host 不可用 → T0 定夺回退路径〔模型直传坐标或内置常用城市表〕，T0 取证输出承载）④ 专属 Host 制度依赖（和风若调整制度需改 .env 配置——零代码变更，风险低）。
- 落地核对清单（v1.4.10）：
  1. ✅ spec.md 新增 REQ-053 正文 + §4 暂缓池「真实天气工具」条目移出注记 + REQ-030 波及注记（描述注记 + 验收 6 改写）+ REQ-031/035 复用注记 + 头部基线注记 v13 + 全局回归基线段天气面（2026-08-22 同批）
  2. ✅ rtm.md 新增 REQ-053 行 + 全局回归基线行「天气面（CHG-016）」注记（2026-08-22 同批）
  3. ✅ changes.md 本条状态更新为已批准 + 定夺结果回填（2026-08-22 同批）
  4. ✅ tag req-baseline-v13（2026-08-22 主会话执行，随批准批次提交落位）
  - 后续动作（随迭代/发布流程承载，不入本清单）：plans/iter-22.md 依本 CHG 定夺经 /mm-iteration-plan 起草报批；生产服务器 .env 增 `AI_CHAT_WEATHER_KEY`/`AI_CHAT_WEATHER_HOST` + 容器重建（随 iter-22 交付，发布记录注记）。
