...
| REQ-020 | 注册与登录（用户名+密码，会话 token；用户名规则 2~32 字符/大小写不敏感唯一）〔CHG-004〕 | P0（CEO 已确认） | design/iter-6（已基线，2026-08-15 CEO 批准） | backend/（FastAPI 骨架 + SQLite 带版本迁移 + 注册/登录/登出/me + HttpOnly Cookie SameSite=Lax + SSE echo 形态验证，T1 已完成）；登录页与路由守卫（T2，未开始） | backend/tests（pytest 20 用例全绿：注册直登/Cookie 属性/大小写不敏感/长短与非法字符拦截/统一错误文案/登出失效/过期 401/封禁 403/库内无明文） | 开发中（T1 后端完成 2026-08-15，待 T2 前端 + 容器化验收） | iter-6（T0/T1/T2） |
