...
# R2 定稿逐字（plans/iter-18-verify.md §1；物理行拼接仅源码行宽合规，字节值与定稿逐字等价）
R2 = (
...
def test_research_指令逐字_与定稿一致():
    """逐字纪律锚点：模块常量与 R2 定稿（测试独立转录）逐字节一致。"""
    assert research_mod.RESEARCH_PROMPT == R2
    assert len(research_mod.RESEARCH_PROMPT) == 370
...
def test_research_帧级断言_规划两搜索综合_逐帧(tmp_path: Path):
    ...
    assert msgs[2] == {"role": "system", "content": research_mod.RESEARCH_PROMPT}  # 指令逐字
...
def test_research_注入位_记忆预置_指令在记忆前_普通回合不含(tmp_path: Path):
    ...
    with research_app(tmp_path, llm, settings_extra={"product_persona": PERSONA}) as (c, seen, _):
        ...
        assert msgs[1]["role"] == "system"
        assert msgs[2] == {"role": "system", "content": research_mod.RESEARCH_PROMPT}
        ...
        assert all(m.get("content") != research_mod.RESEARCH_PROMPT for m in msgs2)
...
def test_research_mode缺省普通回合零影响(tmp_path: Path):
    ...
    with turn_app(tmp_path, llm) as (c, seen):  # 不设 search_key
        ...
        assert evs[-1]["reason"] == "done"
        msgs = json.loads(seen[-1].content.decode())["messages"]
        assert all(m.get("content") != research_mod.RESEARCH_PROMPT for m in msgs)
        # 显式 mode='chat' 等价普通回合
...
def test_research_卫生_指令事件流遥测零key(tmp_path: Path):
    ...
    assert SEARCH_KEY not in research_mod.RESEARCH_PROMPT
    conn = sqlite3.connect(c.app.state.db_path)
    conn.row_factory = sqlite3.Row
